...
class Simulation():
    ...
    def __init__(self):
        ...
        print("To simulate an epidemic outbreak, we must know the populationsize.")
        self.population_size = int(input("---Enter the population size: "))
        print("\nWe must first start by infecting a portion of the population.")
        self.infection_percent = float(input("---Enter the percentage (0-100) ofthe population to initially infect: "))
        self.infection_percent /= 100
        print("\nWe must know the risk a person has to contract the disease whenexposed.")
        ...
        self.infection_duration = int(input("---Enter the duration (in days) ofthe infection: "))
...
class Person():
    ...
    def __init__(self):
        ...
        self.days_infected = 0 #Keeps track of days infected for individualperson
    def infect(self, simulation):
        """Infect a person based on sim conditions"""
        #random number generated must be less than infection_probability toinfect
...
class Population():
    ...
    def initial_infection(self, simulation):
        ...
        #We must round to 0 decimals and cast to an int so we can useinfected_count in a for loop.
    ...
    def graphics(self):
        """A graphical representation for a population. O is healthy, Iinfected, X dead."""
...
𝑑𝑡
= γI --------------(3
transmission rate and rate of recovery (removal)
